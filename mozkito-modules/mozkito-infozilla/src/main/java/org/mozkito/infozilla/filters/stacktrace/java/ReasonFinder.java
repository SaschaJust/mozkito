	...
	public boolean find() {
		...
		if (multiMatch != null) {
			for (final Match match : multiMatch) {
				final String className = JavaStackTraceFilter.lineBreaks.removeAll(match.getGroup(JavaStackTraceFilter.CLASSNAME_GROUP)
				                                                                        .getMatch());
				final String methodName = JavaStackTraceFilter.lineBreaks.removeAll(match.getGroup(JavaStackTraceFilter.METHODNAME_GROUP)
				                                                                         .getMatch());
				String fileName = match.getGroup(JavaStackTraceFilter.FILENAME_GROUP).getMatch();
				if (fileName != null) {
					fileName = JavaStackTraceFilter.lineBreaks.removeAll(fileName);
				}
				...
				if (lineNumberStr != null) {
					lineNumberStr = JavaStackTraceFilter.lineBreaks.removeAll(lineNumberStr).trim();
