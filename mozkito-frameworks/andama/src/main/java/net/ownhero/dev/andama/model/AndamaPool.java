...
public class AndamaPool {
	...
	//	 * {@link AndamaThread} class, e.g. {@link AndamaSource}, to a type
	//	 * determining the input output connectors of the corresponding threads.
	//	 * 
	//	 * @param source
	//	 * @param filterSet
	//	 * @param transformerSet
	//	 * @param sinkSet
	//	 * @return a {@link List} of {@link Tuple} containing a {@link AndamaThread}
	//	 *         and a {@link Type} which suffices to classify the corresponding
	//	 *         {@link AndamaThread}.
	//	 */
	//	@SuppressWarnings({ "unchecked", "rawtypes" })
	//	private LinkedList<Tuple<Class<? extends AndamaThread>, Tuple<Type, Type>>> buildGraph(final Type source,
	//	        Set<Tuple<Type, Type>> filterSet, Set<Tuple<Type, Type>> transformerSet, Set<Tuple<Type, Type>> sinkSet) {
	...
	private void connectThreads() {
		AndamaGraph andamaGraph = AndamaGraph.buildGraph(threads);
		int color = 1;
		
		if (andamaGraph.unique()) {
			if (this.logger.isInfoEnabled()) {
				this.logger.info("Found unique graph repesentation. Using...");
				// TODO display graph
			}
		} else {
			if (this.logger.isInfoEnabled()) {
				this.logger.info("Found %s graph repesentations. Please choose from...", andamaGraph.alternatives());
				// TODO display graph
			}
		}
		...
		//				this.logger
		//				        .error("Currently, only simple graphs are supported (using only 1 class of source). Given: ");
