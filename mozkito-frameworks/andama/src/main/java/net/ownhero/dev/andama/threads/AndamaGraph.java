...
public class AndamaGraph {
	...
	private static File provideResource(final AndamaGroup andamaGroup) {
		...
		if ((cache == null) || (cache.length() <= 1)) {
			...
			if ((cache == null) || (cache.length() <= 1)) {
				/*
				 * try to determine name from common string prefix of thread names e.g. RepositoryReader
				 * RepositoryAnalyzer RepositoryParser RepositoryPersister result in "Repository" -> repository.agl
				 */
