...
public class BranchFactory {
		...
		if (!branchCache.containsKey(name)) {
			if (persistenceUtil == null) {
				...
				final Criteria<RCSBranch> criteria = persistenceUtil.createCriteria(RCSBranch.class)
				                                                    .eq("name", RCSBranch.MASTER_BRANCH_NAME);
	...
	public static RCSBranch getMasterBranch(final PersistenceUtil util) {
		return getBranch(RCSBranch.MASTER_BRANCH_NAME, util);
	}
}
