package de.unisaarland.cs.st.reposuite.infozilla.filters.enumeration;
...
public class EnumerationFilter implements Filter {
	 ...
	 * @param text
	 */
	private void filterLine(int lineNum,
	                        String text) {
	 ...
	 */
	private Enumeration createEnumeration(int startline,
	                                      int endline,
	                                      String text) {
		assert (startline >= 0);
		assert (endline >= 0);
	...
	private List<Enumeration> getCharEnums(String s) {
		...
		for (int i = 0; i < lines.length; i++) {
			...
			if (line.matches(regex_EnumStart)) {
				symbolCount++;
				// Initialize the start of an Enumeration if none was found
				// before!
				if (enumStart < 0) enumStart = i;
				...
				if (foundEnumSymbol.compareTo(lastFoundEnumSymbol) > 0) {
					...
					if (enumEnd < 0) enumEnd = previousEnumLineFound;
					if (symbolCount > 1) {
	...
	private List<Enumeration> getItemizations(String s) {
		...
		for (int i = 0; i < lines.length; i++) {
			...
			if (line.startsWith("- ")) {
				...
				if (itemizeBegin < 0) itemizeBegin = i;
