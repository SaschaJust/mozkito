package de.unisaarland.cs.st.reposuite.infozilla.BugReport.MIME;

import java.util.Map;

import de.unisaarland.cs.st.reposuite.infozilla.DataSources.BugZillaDB.DatabaseConnector;
...
public class MainMimeTyper {
	...
	public static void main(final String[] args) {
		...
		do {
			System.out.println("-------------------------------------------------------------------------------");
			// Fetch a number of Bug Report unique IDs belonging to the
			// following query
			
			String setSelectionQuery = "SELECT DISTINCT bug_id FROM bugzilla_bugs WHERE bug_id > " + startID
			        + " ORDER BY bug_id ASC LIMIT " + args[5];
			...
			dbc.getReportsConcurrent(bugIDs, true, false, false);
			...
			// Output last processed id for the user to he can pick up execution
			// there
			System.out.println("-------------------------------------------------------------------------------");
