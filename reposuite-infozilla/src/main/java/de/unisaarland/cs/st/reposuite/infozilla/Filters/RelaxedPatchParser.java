...
public class RelaxedPatchParser {
	 ...
	 */
	private int findNextIndex(String[] lines,
	                          int start) {
		int found = -1;
	 ...
	 */
	private int findNextHunkHeader(String[] lines,
	                               int start) {
		int found = -1;
	 ...
	 */
	private int findFirstLineBeginningWith(String text,
	                                       String[] lines,
	                                       int start) {
		int found = -1;
		for (int i = start; i < lines.length; i++) {
	 ...
	 */
	private String findFirstLineBeginningWithS(String text,
	                                           String[] lines,
	                                           int start) {
		String found = "";
		for (int i = start; i < lines.length; i++) {
	 ...
	 */
	private List<PatchHunk> findAllHunks(String[] lines,
	                                     int start) {
		...
		while (hasMore) {
			...
			if (hStart == -1) {
				...
				// Kill last newline
				if (hunktext.length() > 1) hunktext = hunktext.substring(0, hunktext.length() - 1);
