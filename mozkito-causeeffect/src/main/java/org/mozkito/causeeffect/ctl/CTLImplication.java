...
public class CTLImplication extends CTLBilateralFormula {
	 ...
	 *         recognized.
	 */
	public static CTLImplication getFromXMLRepresentation(final Element element) {
		assert "CTL-if".equals(element.getNodeName());
	 ...
	 *            Right side of the implication.
	 */
	private CTLImplication(final CTLFormula left, final CTLFormula right) {
		this.left = left;
		this.right = right;
	}
	...
	public Element getXMLRepresentation(final Document xml) {
		final Element ctlXML = xml.createElement("CTL-if");
