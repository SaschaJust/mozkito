...
public class JiraXMLParser {
	...
	public static void handleHistory(final URI historyUri,
		...
		// FIXME JIRA-Tracker parse history is broken
		if (Logger.logWarn()) {
			Logger.warn("Parsing JIRA history is disabled due to schema changes.");
		}
		return;
		
		/*
		 * RawContent rawContent = null; try { rawContent = IOUtils.fetch(historyUri); BufferedReader reader = new
		 * BufferedReader(new StringReader(rawContent.getContent())); SAXBuilder saxBuilder = new
		 * SAXBuilder("org.ccil.cowan.tagsoup.Parser"); Document document = saxBuilder.build(reader); reader.close();
		 * Element rootElement = document.getRootElement(); if (!rootElement.getName().equals("html")) { if
		 * (Logger.logError()) { Logger.error("Error while parsing bugzilla report history (id: " + report.getId() +
		 * "). Root element expectedto have `<html>` tag as root element. Got <" + rootElement.getName() + ">."); }
		 * return; } Element body = rootElement.getChild("body", namespace); if (body == null) { if (Logger.logError())
		 * { Logger.error("Error while parsing bugzilla report history (id: " + report.getId() +
		 * "). No <body> tag found."); } return; } Element nextElem = getElement(body, namespace, "div", "id",
		 * "main-content"); nextElem = getElement(nextElem, namespace, "div", "class", "active-area"); nextElem =
		 * getElement(nextElem, namespace, "div", "id", "primary"); nextElem = getElement(nextElem, namespace, "div",
		 * "class", "content"); nextElem = getElement(nextElem, namespace, "div", "id", "activitymodule"); nextElem =
		 * getElement(nextElem, namespace, "div", "class", "mod-content"); nextElem = getElement(nextElem, namespace,
		 * "div", "id", "issue_actions_container"); List<Element> changeHistoryItems = nextElem.getChildren("div",
		 * namespace); for (Element changeHistoryItem : changeHistoryItems) { if (
		 * changeHistoryItem.getAttributeValue("class").equals("issue-data-block" )) { List<Element> actionContainers =
		 * changeHistoryItem.getChildren("div", namespace); for (Element actionContainer : actionContainers) { Person
		 * author = null; DateTime timestamp = null; String oldValue = null; String newValue = null; Element
		 * actionDetails = getElement(actionContainer, namespace, "div", "class", "action-details"); List<Element> as =
		 * actionDetails.getChildren("a", namespace); if ((as != null) && (as.size() < 2)) { if (Logger.logError()) {
		 * Logger.error("Error while parsing jira history (id: " + report.getId() +
		 * "). HTML structure unknown: could not find second a tag in actionDetails" ); } return; } Element
		 * actionDetailsA = as.get(1); String email = actionDetailsA.getAttributeValue("rel"); String fullname =
		 * actionDetailsA.getText(); author = new Person(null, fullname, email); Element date =
		 * getElement(actionDetails, namespace, "span", "class", "date"); String dateString = date.getText(); if
		 * (dateString != null) { timestamp = DateTimeUtils.parseDate(dateString, dateTimeHistoryFormatRegex); } Element
		 * actionBody = null; try { actionBody = getElement(actionContainer, namespace, "div", "class",
		 * "changehistory action-body"); } catch (NoSuchElementException e) { actionBody = getElement(actionContainer,
		 * namespace, "div", "class", "action-body"); } Element table = actionBody.getChild("table", namespace); if
		 * (table == null) { if (Logger.logError()) { Logger.error("Error while parsing jira history (id: " +
		 * report.getId() + "). HTML structure unknown: could not find table in actionBody"); } return; } Element tbody
		 * = table.getChild("tbody", namespace); if (tbody == null) { if (Logger.logError()) {
		 * Logger.error("Error while parsing jira history (id: " + report.getId() +
		 * "). HTML structure unknown: could not find tbody in actionBody"); } return; } List<Element> trs =
		 * tbody.getChildren("tr", namespace); HistoryElement hElement = new HistoryElement(report.getId(), author,
		 * timestamp); for (Element tr : trs) { if (tr == null) { if (Logger.logError()) {
		 * Logger.error("Error while parsing jira history (id: " + report.getId() +
		 * "). HTML structure unknown: could not find tr in actionBody"); } return; } String fieldString =
		 * getElement(tr, namespace, "td", "class", "activity-name").getText() .trim(); oldValue = getElement(tr,
		 * namespace, "td", "class", "activity-old-val").getText().trim(); newValue = getElement(tr, namespace, "td",
		 * "class", "activity-new-val").getText().trim(); if (fieldString.equals("Status")) {
		 * hElement.addChangedValue("status", getStatus(oldValue), getStatus(newValue)); } else if
		 * (fieldString.equals("Resolution")) { hElement.addChangedValue("resolution", getResolution(oldValue),
		 * getResolution(newValue)); } else if (fieldString.equals("Priority")) { hElement.addChangedValue("priority",
		 * getPriority(oldValue), getPriority(newValue)); } } if (!hElement.isEmpty()) {
		 * report.addHistoryElement(hElement); } } } } } catch (NoSuchElementException e) { if (Logger.logError()) {
		 * Logger.error ("Error while parsing jira history. HTML structure unknown: " + e.getMessage(), e); if
		 * ((rawContent != null) && Logger.logError()) { Logger.error("RAW REPORT DATA:");
		 * Logger.error(rawContent.getContent()); } } return; } catch (FetchException e) { if (Logger.logError()) {
		 * Logger.error("Error while fetching jira history. URL not found: " + e.getMessage(), e); } return; }
		 */
	}
