 ...
 *
 */
public class ScoringTransactionProcessor extends RepoSuiteTransformerThread<RCSTransaction, MapScore> {
	 ...
	 */
	public ScoringTransactionProcessor(final RepoSuiteThreadGroup threadGroup, final MappingSettings settings,
	        final MappingFinder finder, final PersistenceUtil persistenceUtil) {
		super(threadGroup, ScoringTransactionProcessor.class.getSimpleName(), settings);
	...
	public void run() {
		try {
			...
			while (!isShutdown() && ((transaction = read()) != null)) {
				for (Report report : this.mappingFinder.getCandidates(transaction, Report.class)) {
					if (Logger.logDebug()) {
